class Solution {
    // time complexity is O(n*m)^2, we are going through each cell in the grid which is n*m.
    // and for each cell we can visit n*m cells to find the word, so (n*m)*(n*m) = (n*m)^2
